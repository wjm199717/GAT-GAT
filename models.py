...
#tf.get_collection是从一个集合中取出所有变量，返回一个列表
#tf.GraphKeys是tensorflow（图）上所有集合（一个集合包含了所属的变量）的总称，
#tensorflow就是将一个模块看作图，GLOBAL_VARIABLES就是一个集合名称
...
#第一个参数 function 对参数序列中的每一个元素调用 function 函数，
#返回包含每次 function 函数返回值的新列表
        #self.structural_head_config=16
...
#tf.nn.fixed_unigram_candidate_sampler进行负例采样
#在不少任务中需要进行负例采样来降低训练难度，加速训练            
            self.proximity_neg_samples.append(tf.nn.fixed_unigram_candidate_sampler(
                true_classes=proximity_labels[t],# 待采样真实数据,通常是数据ID完整集,比如词ID
                num_true=1,# 待采样真实数据类别总数，如词表大小
                num_sampled=FLAGS.neg_sample_size,# 采样个数
                unique=False,# 随机样本是否唯一，通常是False，可以重复出现
                range_max=len(self.degrees[t]),# 随机最大的数
                distortion=0.75,
                unigrams=self.degrees[t].tolist())[0])# 采样概率，如词表中每个词出现的频率作为对应概率，长度要和V一致
    ...
    def build_net(self, attn_head_config, attn_layer_config, temporal_head_config, temporal_layer_config,
        ...
        for i in range(0, len(attn_layer_config)):
            if i > 0:
                ...
                sparse_inputs = False
            self.structural_attention_layers.append(layers.StructuralAttentionLayer(input_dim=input_dim,
...
#python3中的map返回的不是一个列表，而是一个可迭代对象，需要list()转换成列表。
#python3中的range()返回的是一个可迭代对象，需要list()转换成列表。
        for i in range(0, len(temporal_layer_config)):
            if i > 0:
                input_dim = temporal_layer_config[i - 1]
            temporal_layer = layers.TemporalAttentionLayer(input_dim=input_dim, n_heads=temporal_head_config[i],
...
#            print(input_list)
        ...
        for t in range(0, self.num_time_steps):
            ...
            #tf.concat(attn_outputs, axis=0) = (3,24,128)
            #structural_outputs = (24, 3, 128)
